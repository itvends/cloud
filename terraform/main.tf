# vim: set filetype=terraform
# terraform/itvends.tf
# itvends/cloud
#
# Base Infrastructure Provisioning
#

module "dogfood" {
  source = "./dogfood"
  region = "us-west-2"
  name   = "It Vends"

  tags = {
    Tenant = "ItVends"
  }

  domain       = "itvends.cloud"
  ldapou       = ""
  alias        = "itvends"
  workmail_arn = "767827555133:organization/m-d07a978964544577b1c6103b63824f5b"

  admin  = "eugene"
  pubkey = "ssh-rsa "
}

module "sokkeli" {
	source = "./sokkeli"

	meta-prefix = "itvends"
	meta-tags = {
		Tenant = "ItVends"
	}

	admin-username = "eugene"
  admin-pubkey = "ssh-rsa "

	axle-location = "eastus"
	axle-cidr = "10.172.0.0/18"

	spoke-location = ["eastus", "westus", "westeurope", "eastasia"]
	spoke-cidr = "10.172.64.0/18"
}


